...
class ClassSpec(Spec):
    ...
    def get_containing_relations(self):
        ...
        spec_containers = dict([(spec.name, spec) 
                                for spec in self.containing_components])
        # remote class and relation names for this spec
        spec_relations = dict([(v.remote_classname, v.name)
                                for v in self.relationships.values()])

        # find container specs that ARE NOT direct ancestors
        cntr_ancestors = set(spec_containers.keys())
        rel_ancestors = set(spec_relations.keys())
        indirect_relations = list(cntr_ancestors.difference(rel_ancestors))
        direct_relations = list(cntr_ancestors.intersection(rel_ancestors))
        ...
        for ancestor_name in indirect_relations:
            ...
            for direct_name in direct_relations:
                ...
                target_name = spec_containers.get(direct_name)
                ancestor_rel = self.find_target_spec_relation(target_name, 
                                                              ancestor_name)
                container_rels[ancestor_name] = ancestor_rel
    ...
    def find_target_spec_relation(self, spec, target):
        ...
        # remote class and relation names for this spec
        spec_relations = dict([ (v.remote_classname, v.name) 
                               for v in spec.relationships.values()])
        ...
        if relname:
            ...
            # container class name and specs, some of which may be indirect ancestors
            spec_containers = dict([(spec.name, spec) 
                                    for spec in spec.containing_components])
            ...
            for direct_name in direct_relations:
                target_name = spec_containers.get(direct_name)
                relname = self.find_target_spec_relation(target_name, target)
                if relname: 
                    return relname
        return None
